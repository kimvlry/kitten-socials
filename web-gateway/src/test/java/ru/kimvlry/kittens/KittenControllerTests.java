...
import ru.kimvlry.kittens.controller.KittenController;
import ru.kimvlry.kittens.common.KittenDto;
import ru.kimvlry.kittens.common.KittenBreed;
import ru.kimvlry.kittens.common.KittenCoatColor;
...
class KittenControllerTests {
    private String messageBrokerQueueName = "kittens.request";
    ...
    void searchKittens_returnsKittensPage() throws Exception {
        ...
        when(messageClient.sendPageRequest(eq(messageBrokerQueueName), eq("SEARCH_KITTENS"), any(), eq(KittenDto.class))).thenReturn(page);
    ...
    void searchKittens_returnsEmptyPage() throws Exception {
        ...
        when(messageClient.sendPageRequest(eq(messageBrokerQueueName), eq("SEARCH_KITTENS"), any(), eq(KittenDto.class)))
    ...
    void createKitten_returnsCreatedKitten() throws Exception {
        ...
        when(messageClient.sendRequest(eq(messageBrokerQueueName), eq("CREATE_KITTEN"), any(), eq(KittenDto.class))).thenReturn(responseDto);
    ...
    void deleteKitten_returnsNoContent() throws Exception {
        ...
        verify(messageClient).sendRequest(eq(messageBrokerQueueName), eq("DELETE_KITTEN"), eq(99L), eq(KittenDto.class));
    }
}